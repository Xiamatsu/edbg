...
#define FLASH_CR_OBPG          (1 << 4)
#define FLASH_CR_OBER          (1 << 5)
#define FLASH_CR_OBWEN         (1 << 9)
...
#define OPTIONS_OPTR           0x1ffff800
#define OPTIONS_USER           0x1ffff804
#define OPTIONS_WRPR0          0x1ffff808
#define OPTIONS_WRPR1          0x1ffff80c
...
{
  { 0x19090410, "gd32e23x", "GD32E230x8", 64*1024 },
};
...
static void target_select(target_options_t *options)
{
  ...
  for (int i = 0; i < ARRAY_SIZE(devices); i++)
  {
    ...
    locked = (0xa5 != (dap_read_word(OPTIONS_OPTR) & FLASH_OPTR_RDP_MASK));
...
{
  uint32_t value = (dap_read_word(OPTIONS_OPTR) & 0x0000) | 0x44bb;
...
{
  uint32_t value = (dap_read_word(OPTIONS_OPTR) & 0x0000) | 0x5aa5;

  dap_write_word_req(FLASH_OPTR, value);
  dap_write_word_req(FLASH_CR, FLASH_CR_OBPG);
  dap_write_word_req(0x40022080, 0);
  dap_transfer();
